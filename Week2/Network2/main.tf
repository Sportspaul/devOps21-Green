...
resource "azurerm_network_interface" "Sub3-interface" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.pub-ip3.id
  }
}

resource "azurerm_public_ip" "pub-ip3" {
  name                = "pub-ip3"
  resource_group_name = data.azurerm_resource_group.rg.name
  location            = data.azurerm_resource_group.rg.location
...
resource "azurerm_public_ip" "pub-ip4" {
  name                = "pub-ip4"
  resource_group_name = data.azurerm_resource_group.rg.name
  location            = data.azurerm_resource_group.rg.location
  allocation_method   = "Dynamic"
...
resource "azurerm_network_interface" "Sub4-interface" {
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.pub-ip4.id
  }
}
...
resource "azurerm_linux_virtual_machine" "VM4" {
  ...
  location            = data.azurerm_resource_group.rg.location
  size                = "Standard_B1ls"
  admin_username      = "vm4_admin"
  network_interface_ids = [
    azurerm_network_interface.Sub4-interface.id
  ]
